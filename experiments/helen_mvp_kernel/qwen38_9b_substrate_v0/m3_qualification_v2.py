#!/usr/bin/env python3
"""
M3 HELEN SUBSTRATE QUALIFICATION V2 — per-item grounding scope on discrim items (sole delta vs v1.2, per DISPOSITION.md unblock path) — frozen-packet ladder C_NULL -> 2B -> 9B.
...
def run_substrate(name: str, model_path: str | None) -> dict:
    ...
    try:
        ...
        for i, (arg, keys) in enumerate(DISCRIM):
            c = get(f"D{i+1}",
                    f"GROUNDING SCOPE FOR THIS ITEM ONLY: the argument below is the "
                    f"object of analysis. It is independent of the SOURCE PACKET; "
                    f"R1 grounding does not apply to it. "
        ...
        if raw_log:
            (HERE / f"raw_v2_{name}.ndjson").write_text(
...
def main():
    ...
    out = {"packet_hash": PACKET_HASH, "verifier_hash": VERIFIER_HASH,
           ...
           "rows": rows}
    (HERE / "M3_SUBSTRATE_RECEIPT_V2.json").write_text(json.dumps(out, indent=2))
    print("M3_SUBSTRATE_RECEIPT_V2.json written")
    print("DONE_QUAL")
