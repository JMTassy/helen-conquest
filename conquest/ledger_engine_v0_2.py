#!/usr/bin/env python3
"""
Conquest Ledger Engine v0.2

Validated pressure kernel + smart entropy-aware heuristic.
Tests: if entropy-aware strategy improves win/collapse rates.
...
class LedgerEngine:
    ...
    def simulate_session(
        ...
        for turn in range(1, max_turns + 1):
            # Smart entropy-aware heuristic (v0.2)
            # Prioritizes stability; avoids overexpansion
            if state.stability <= 0:
                action = "rest"  # Critical: must stabilize
            elif state.entropy > 0.8:
                action = "rest"  # High chaos: recover
            elif state.entropy > 0.5:
                action = "duel"  # Moderate chaos: cautious territory claiming
            elif state.territory >= 6:
                # Goal reached — but forced greedy expansion (forces late-game risk)
                if state.zols > 300:
                    action = "expand"  # Mandatory expansion (high entropy risk)
                elif state.entropy > 0.5:
                    action = "rest"  # If broke or high entropy, recover
                else:
                    action = "duel"  # Otherwise keep claiming
            elif state.territory >= 3:
                # Mid-game: push toward 6 territories cautiously
                action = "duel"
            else:
                # Early game: build knowledge or claim first territory
                action = "duel" if self.roll_d6() >= 3 else "knowledge_check"
        ...
        # Determine winner (reached 6 territories AND 100+ knowledge without collapse)
        # This forces knowledge-building as a requirement, not a trap
        winner = (state.territory >= 6 and state.knowledge >= 100 and collapse_turn == -1)
